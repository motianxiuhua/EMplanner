...
void ReferenceLineProvider::Provide(
    ...
    const std::unordered_map<std::string, double> &referenceline_params)

{
  ...
  //截取初始参考线
  GetReferenceLine(frenet_path_, frenet_path_.host_match_point().index,
                   raw_reference_line_);
  //参考线平滑
  // ReferenceLineSmootherConfig smoother_config; //平滑配置
  std::unique_ptr<ReferenceLineSmoother> smoother =
      std::make_unique<ReferenceLineSmoother>(referenceline_params);
  smoother->Smooth(raw_reference_line_, reference_line);

  //计算heading和kappa
  CalHeadingAndKappa(reference_line);
...
void ReferenceLineProvider::GetReferenceLine(
  ...
  //% 匹配点前面的点太少了，不够150个
  else if (host_match_point_index + 151 > len) {
    start_index = len - 181;
  }
  //正常情况
  else {
  ...
  // std::vector<ReferencePoint>::const_iterator it_start1_nothing =
  //     frenet_path.reference_points().begin();
  // std::vector<ReferencePoint>::const_iterator it_start_add_zero =
  //     frenet_path.reference_points().begin() + 0;
  ...
  std::vector<ReferencePoint>::const_iterator it_end =
      frenet_path.reference_points().begin() + start_index + 181;
  // TODO 非常奇怪，frenet_path 0点没有问题，frenet_path.reference_points().begin()没有问题
  // 但是it_start有问题，而且只有不加任何值或者+0才会有问题，下面的初始化之后，reference_points 0点发生变化
  std::vector<ReferencePoint> reference_points(it_start, it_end);
  if (start_index == 0){
    reference_points[0] = frenet_path.reference_points()[0];
  }//补偿start_index为0时，reference_points 0点不正确的情况
...
// TODO 不需要算heading route信息可以得到，route的信息也不只是xy
void ReferenceLineProvider::RoutingPathToFrenetPath(
  ...
  */
  //前向差分，后向差分，差分中值
  //求heading
  //heading部分在carla中能够直接给出值所以不需要再计算了
  ...
  for (int i = 0; i < size - 1; i++) {
    heading_diff.push_back(points[i + 1].heading - points[i].heading);
  }
...
void ReferenceLineProvider::CalHeadingAndKappa(
    ReferenceLine &frenet_path) {
  auto points = frenet_path.reference_points();
  int size = points.size();
  /*此部分可以通过构造2d矢量类，来统一计算，见apollo common::math::Vec2D
  此处使用的是vector编程，后面再考虑使用eigen怎么操作
  */
