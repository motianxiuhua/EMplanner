...
PathTimeGraph::PathTimeGraph(const ReferenceLine &reference_line,
                             ...
                             const std::unordered_map<std::string, double> &emplaner_conf) {
...
void PathTimeGraph::SetStaticObstaclesSl() {
  std::vector<MapPoint> map_points;
  int size = static_obstacles_.size();

  map_points.resize(size);
  for (int i = 0; i < size; i++) {
    map_points[i].x = static_obstacles_[i].center_point.x;
    map_points[i].y = static_obstacles_[i].center_point.y;
  }
  //定义变量，用于存储计算结果
  std::vector<ReferencePoint> obstacles_match_points, obstacles_project_points;

  ReferenceLineProvider::FindMatchAndProjectPoint(reference_line_, map_points,
                                                  0, 5, obstacles_match_points,
  ...
  for (int i = 0; i < size; i++) {
    traj_points[i] = static_obstacles_[i].center_point;
  }
  ...
  //计算障碍物的边界，这里之所以需要利用循环来写是因为函数的参数是固定的
  //TODO之后改进可以做个模板函数
  map_points.resize(8);
  traj_points.resize(8);
  box_sl_static_obstacles_.resize(size);
  for(int i = 0; i < size; i++){
    for(int j = 0; j < 8; j++){
      map_points[j].x = static_obstacles_[i].collision_box[j].x;
      map_points[j].y = static_obstacles_[i].collision_box[j].y;
      ReferenceLineProvider::FindMatchAndProjectPoint(reference_line_, map_points,
                                                  0, 5, obstacles_match_points,
                                                  obstacles_project_points);
    }
    traj_points = static_obstacles_[i].collision_box;
    Cartesian2Frenet(reference_line_, sl_reference_line_, traj_points,
                   obstacles_match_points, obstacles_project_points,
                   box_sl_static_obstacles_[i]);
  }
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (points_fcs[i].ds_dt < 1e-3) {
      ...
      points_fcs[i].ddl_ds = (points_fcs[i].ddl_dt - points_fcs[i].dl_ds *
            points_fcs[i].dds_dt) / pow(points_fcs[i].ds_dt, 2);
    }
  }
}

void PathTimeGraph::CalcProjPoint(
    const SLPoint &sl_point, const std::vector<SLPoint> &sl_reference_line,
...
{
  int size = points_fcs.size();
  points_wcs.resize(size);
  for (int i = 0; i < size; i++) {
    ReferencePoint project_point;
    CalcProjPoint(points_fcs[i], sl_reference_line,
                  reference_line.reference_points(), project_point);
    ...
    Eigen::Vector2d point = vector_proj + points_fcs[i].l * nor;
    double heading =
...
double PathTimeGraph::CalcPathCost(SLPoint point_satrt, SLPoint point_end) {
  ...
  for (int i = 0; i < 10; i++) { //不包含起点包含终点
    ...
    //参考线代价
    cost_ref = cost_ref + config_["dp_cost_ref"] * pow(quintic_path_point.l, 2);
    ...
    if (min_d > 25)
      ...
      cost_collision = cost_collision + 1000;
    else
      cost_collision = cost_collision + config_["dp_cost_collision"];

    //TODO 后续超车可以像lattice planner一样左右超车给不同的cost因为一般超车倾向于从左侧，或者看车道的位置
  }
  //三个代价不是一个数量级
...
void PathTimeGraph::CreateSamplingPoint() {
                   ...
                   */
  double row = config_["dp_sample_rows"];
  double col = config_["dp_sample_cols"];
  double sample_s = config_["dp_sample_s"];
  double sample_l = config_["dp_sample_l"];
...
void PathTimeGraph::DpPathInterpolation() {
...
void PathTimeGraph::GenerateConvexSpace() {
  ...
  for (int i = 0; i < static_obstacles_.size(); i++) {
    //TODO这里需要改成collsion box的s l
    double obs_s_min = sl_static_obstacles_[i].s;
    double obs_s_max = sl_static_obstacles_[i].s;
    double obs_l_min = sl_static_obstacles_[i].l;
    double obs_l_max = sl_static_obstacles_[i].l;
    for (auto &box_point : box_sl_static_obstacles_[i])
    {
...
bool PathTimeGraph::PathQuadraticProgramming() {
  ...
  */
  ...
  lb(0) = sl_plan_start_.l;
  lb(1) = sl_plan_start_.dl_ds;
  lb(2) = sl_plan_start_.ddl_ds;
  ub(0) = lb(0);
  ub(1) = lb(1);
  ub(2) = lb(2);
  ...
  qp_path_points_.resize(n);
  for (int i = 0; i < n; i++) {
    qp_path_points_[i].s = sl_plan_start_.s + ds * i;
...
void PathTimeGraph::QpPathInterpolation() {
...
void PathTimeGraph::GeneratePlaningPath() 
{
  ...
  Frenet2Cartesian(reference_line_, sl_reference_line_, 
                   qp_path_points_dense_, planning_path_points);
