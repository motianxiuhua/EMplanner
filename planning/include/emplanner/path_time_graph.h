...
public:
  ...
  static void CalcProjPoint(const SLPoint &sl_point,
  ...
  void SetStaticObstaclesSl(); //存在问题

  // 4.dp主算法相关生成采样点
  void
  CreateSamplingPoint(); //对SamplePoints进行操作，调试完毕
...
private:
  ...
  std::vector<std::vector<SLPoint>> box_sl_static_obstacles_; //
  std::vector<ObstacleInfo> static_obstacles_;       //原始静态障碍物
  ...
  ReferenceLine planning_path_; //路径规划后的世界坐标系轨迹
